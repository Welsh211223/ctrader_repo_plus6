...
if (-not $__gotLock) {
  ...
  Send-DiscordInfo "ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚Â¨ ctrader paper run SKIPPED (another instance running)."
  try { Stop-Transcript | Out-Null } catch {}
  exit 2
}

$__sw = [System.Diagnostics.Stopwatch]::StartNew()
Send-DiscordInfo ("ÃƒÆ’Ã‚Â°Ãƒâ€¦Ã‚Â¸Ãƒâ€¦Ã‚Â¸Ãƒâ€šÃ‚Â¦ ctrader paper run STARTED on {0} (`"{1}`")" -f $env:COMPUTERNAME, $__stamp)
