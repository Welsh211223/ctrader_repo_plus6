...
# ASCII-only, WinPS 5.1 safe operate-ctrader.ps1
$operate = @'
[CmdletBinding()]
param(
...
function Notify-Discord([string]$Message){
  # Use env var to avoid quoting issues
  $py = Get-Python
...
function Schedule-DailyPaper([string]$At,[switch]$AssumeYes){
  ...
  $runAt = Get-Date -Hour $hh -Minute $mm -Second 0  # DateTime for -At
...
if ($CommitAndPush){
  ...
  if (-not [string]::IsNullOrWhiteSpace($s)){
    git commit -m "fix(ops): ASCII-only operate-ctrader.ps1; remove emoji and bad quoting"
    git push
...
OK "operate-ctrader.ps1 repaired (ASCII-only)."
